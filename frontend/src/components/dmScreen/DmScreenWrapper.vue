<template>
  <div class="dm-screen-wrapper">t
    <div class="dm-screen-flow-container" :class="{
    ...
    }" :style="canvasBackgroundStyle" @dragover="handleDragOver" @drop="handleDrop">
      <VueFlow v-if="dmScreen" :key="`vueflow-${dmScreen.id}`" ref="vueFlowRef" :nodes="nodes" :edges="[]"
        :node-types="nodeTypes" :default-viewport="{ zoom: 1, x: 0, y: 0 }" :min-zoom="0.2" :max-zoom="4"
        :fit-view-on-init="false" :zoom-on-double-click="false" class="dm-screen-flow" @nodes-change="onNodesChange"
        @node-drag-start="onNodeDragStart" @node-drag="onNodeDrag" @node-drag-stop="onNodeDragStop"
        @node-click="onNodeClick" @pane-click="onPaneClick" @pane-context-menu="handlePaneContextMenu"
        ...
        <GridNode v-if="gridOptions.showGrid" :grid-size="gridOptions.gridSize || 50"
          :grid-color="gridOptions.gridColor || '#ffffff'" :grid-opacity="gridOptions.gridOpacity || 0.6"
          :line-width="gridOptions.gridLineWidth || 1" :offset-x="gridOptions.offsetX || 0"
          :offset-y="gridOptions.offsetY || 0" :show-major-grid-lines="gridOptions.showMajorGridLines !== false"
          :major-grid-interval="gridOptions.majorGridInterval || 5"
          :major-grid-color="gridOptions.majorGridColor || '#ffffff'" :feet-per-square="gridOptions.feetPerSquare || 5"
          :show-scale-indicator="true" />

        <!-- Movement Distance Display (shown while dragging) -->
        <Panel v-if="isDragging && movementDistance && movementDistance.squares > 0" position="top-center"
          class="movement-display-panel">
        ...
        <Panel v-if="!isPortalMode" position="top-center" class="vtt-toolbar-panel">
          <VttToolbar ref="vttToolbarRef" :show-grid="gridOptions.showGrid !== false"
            :snap-to-grid="gridOptions.snapToGrid !== false" :feet-per-square="gridOptions.feetPerSquare || 5"
            :diagonal-rule="localDiagonalRule" @tool-change="handleVttToolChange" @toggle-grid="handleToggleGrid"
            @toggle-snap="handleToggleSnap" @diagonal-rule-change="handleDiagonalRuleChange" />
        </Panel>
        ...
        <Panel v-if="!isPortalMode" position="bottom-center" class="unified-bottom-panel">
          <div class="unified-toolbar">
            ...
            <div class="toolbar-right">
              <v-btn v-if="hasActivePortal" icon size="small" variant="text" color="white" class="mr-1"
                @click="handleSendToPortal">
                <v-icon>mdi-cast</v-icon>
                <v-tooltip activator="parent" location="top">Send to Portal</v-tooltip>
              </v-btn>
              <LayerControl :dm-screen-id="dmScreen.id" :library-id="dmScreen.libraryId"
                :show-grid="gridOptions.showGrid !== false" @layer-select="handleLayerSelect"
      ...
      <MeasurementRuler ref="measurementRulerRef" :is-active="currentVttTool === 'measure'"
        :grid-size="gridOptions.gridSize || 50" :feet-per-square="gridOptions.feetPerSquare || 5"
        :diagonal-rule="localDiagonalRule" :show-path-squares="true"
        :send-to-portal="gridOptions.showMeasurementsOnPortal === true" @measurement-end="handleMeasurementEnd" />
      ...
      <svg
        ...
        <g v-if="showMovementTrail && dragStartPosition && dragCurrentPosition">
          <!-- Trail glow -->
          <line :x1="toScreenX(dragStartPosition.x)" :y1="toScreenY(dragStartPosition.y)"
            :x2="toScreenX(dragCurrentPosition.x)" :y2="toScreenY(dragCurrentPosition.y)"
            stroke="rgba(34, 197, 94, 0.4)" stroke-width="16" stroke-linecap="round" />
          <!-- Main trail line -->
          <line :x1="toScreenX(dragStartPosition.x)" :y1="toScreenY(dragStartPosition.y)"
            :x2="toScreenX(dragCurrentPosition.x)" :y2="toScreenY(dragCurrentPosition.y)" stroke="#22c55e"
            stroke-width="4" stroke-linecap="round" stroke-dasharray="12,6">
            <animate attributeName="stroke-dashoffset" from="0" to="18" dur="0.5s" repeatCount="indefinite" />
          </line>
          <!-- Start point -->
          <circle :cx="toScreenX(dragStartPosition.x)" :cy="toScreenY(dragStartPosition.y)" r="10" fill="#22c55e"
            stroke="white" stroke-width="3" />
          <!-- Current position point -->
          <circle :cx="toScreenX(dragCurrentPosition.x)" :cy="toScreenY(dragCurrentPosition.y)" r="10" fill="#22c55e"
            stroke="white" stroke-width="3" />
          <!-- Distance label while dragging -->
          <g v-if="movementDistance && movementDistance.squares > 0"
            :transform="`translate(${(toScreenX(dragStartPosition.x) + toScreenX(dragCurrentPosition.x)) / 2}, ${(toScreenY(dragStartPosition.y) + toScreenY(dragCurrentPosition.y)) / 2 - 25})`">
            <rect x="-40" y="-14" width="80" height="28" rx="6" fill="rgba(34, 197, 94, 0.95)" stroke="white"
              stroke-width="2" />
            <text x="0" y="6" text-anchor="middle" fill="white" font-size="14" font-weight="bold"
        ...
        <g v-if="movementTrail && movementTrail.segments.length > 0 && !isDragging">
          ...
          <g v-for="(segment, idx) in movementTrail.segments" :key="idx">
            <!-- Trail glow -->
            <line :x1="toScreenX(segment.startX)" :y1="toScreenY(segment.startY)" :x2="toScreenX(segment.endX)"
              :y2="toScreenY(segment.endY)" stroke="rgba(251, 191, 36, 0.3)" stroke-width="14" stroke-linecap="round" />
            <!-- Main trail line -->
            <line :x1="toScreenX(segment.startX)" :y1="toScreenY(segment.startY)" :x2="toScreenX(segment.endX)"
              :y2="toScreenY(segment.endY)" stroke="#fbbf24" stroke-width="3" stroke-linecap="round"
              stroke-dasharray="10,5" />
            <!-- Waypoint marker at start -->
            <circle :cx="toScreenX(segment.startX)" :cy="toScreenY(segment.startY)" r="8" fill="#fbbf24" stroke="white"
              stroke-width="2" />
            <!-- Segment number -->
          ...
          <!-- Final endpoint -->
          <circle v-if="movementTrail.segments.length > 0"
            :cx="toScreenX(movementTrail.segments[movementTrail.segments.length - 1].endX)"
            :cy="toScreenY(movementTrail.segments[movementTrail.segments.length - 1].endY)" r="10" fill="#fbbf24"
            stroke="white" stroke-width="3" />
          <!-- Total distance label near final point -->
          <g v-if="movementTrail.totalDistance.feet > 0"
        ...
        <g v-if="activePing" class="ping-indicator">
          <circle :cx="toScreenX(activePing.x)" :cy="toScreenY(activePing.y)" r="30" fill="none" stroke="#ef4444"
            stroke-width="4" class="ping-ring ping-ring-1" />
          <circle :cx="toScreenX(activePing.x)" :cy="toScreenY(activePing.y)" r="50" fill="none" stroke="#ef4444"
            stroke-width="3" class="ping-ring ping-ring-2" />
          <circle :cx="toScreenX(activePing.x)" :cy="toScreenY(activePing.y)" r="70" fill="none" stroke="#ef4444"
            stroke-width="2" class="ping-ring ping-ring-3" />
          <circle :cx="toScreenX(activePing.x)" :cy="toScreenY(activePing.y)" r="12" fill="#ef4444" stroke="white"
            stroke-width="3" />
        </g>
        ...
        <g v-if="portalMeasurementLines.length > 0 && isPortalMode" class="portal-measurements">
          <g v-for="(line, idx) in portalMeasurementLines" :key="line.id || idx">
            <!-- Line glow -->
            <line :x1="toScreenX(line.startX)" :y1="toScreenY(line.startY)" :x2="toScreenX(line.endX)"
              :y2="toScreenY(line.endY)" stroke="rgba(255, 212, 59, 0.3)" stroke-width="14" stroke-linecap="round" />
            <!-- Main line -->
            <line :x1="toScreenX(line.startX)" :y1="toScreenY(line.startY)" :x2="toScreenX(line.endX)"
              :y2="toScreenY(line.endY)" stroke="#fbbf24" stroke-width="3" stroke-linecap="round"
              stroke-dasharray="10,5" />
            <!-- Waypoints -->
            <circle :cx="toScreenX(line.startX)" :cy="toScreenY(line.startY)" r="8" fill="#fbbf24" stroke="white"
              stroke-width="2" />
          </g>
          <!-- Final waypoint -->
          <circle v-if="portalMeasurementLines.length > 0"
            :cx="toScreenX(portalMeasurementLines[portalMeasurementLines.length - 1].endX)"
            :cy="toScreenY(portalMeasurementLines[portalMeasurementLines.length - 1].endY)" r="8" fill="#fbbf24"
            stroke="white" stroke-width="2" />
          <!-- Total distance label -->
          <g v-if="portalMeasurementTotal > 0"
            ...
            <rect x="-45" y="-16" width="90" height="32" rx="8" fill="rgba(34, 197, 94, 0.95)" stroke="white"
              stroke-width="2" />
            <text x="0" y="6" text-anchor="middle" fill="white" font-size="14" font-weight="bold"
              font-family="JetBrains Mono, monospace">
      ...
      <v-card>
        ...
        <v-card-text class="pa-4">
          ...
          <div class="mb-6">
            ...
            <v-switch v-model="localGridOptions.showGrid" label="Show Grid" color="primary" density="compact"
              hide-details class="mb-3" />

            <!-- Grid Size with scale info -->
            <div class="d-flex align-center gap-2 mb-3">
              <v-slider v-model.number="localGridOptions.gridSize" label="Grid Size" min="20" max="100" step="5"
                thumb-label hide-details class="flex-grow-1" />
              <v-text-field v-model.number="localGridOptions.gridSize" type="number" density="compact" hide-details
                style="max-width: 80px;" suffix="px" />
            </div>

            <!-- Feet per Square (D&D 5e scale) -->
            <div class="d-flex align-center gap-2 mb-3">
              <v-select v-model.number="localGridOptions.feetPerSquare" label="Scale (1 square =)" :items="[
                { value: 5, title: '5 ft (Standard)' },
                { value: 10, title: '10 ft (Large scale)' },
                { value: 2.5, title: '2.5 ft (Half scale)' },
              ]" item-value="value" item-title="title" density="compact" hide-details class="flex-grow-1" />
            </div>

            <!-- Grid Color -->
            <div class="d-flex align-center gap-2 mb-3">
              <span class="text-body-2" style="min-width: 80px;">Grid Color</span>
              <input v-model="localGridOptions.gridColor" type="color" class="color-picker" />
              <v-text-field v-model="localGridOptions.gridColor" density="compact" hide-details
                style="max-width: 100px;" />
            </div>

            <!-- Grid Opacity -->
            <div class="d-flex align-center gap-2 mb-3">
              <v-slider v-model.number="localGridOptions.gridOpacity" label="Grid Opacity" min="0.1" max="1" step="0.05"
                thumb-label hide-details class="flex-grow-1" />
            ...
            <v-switch v-model="localGridOptions.snapToGrid" label="Snap to Grid (center to center)" color="primary"
            ...
            <div class="d-flex gap-2 mb-3">
              <v-text-field v-model.number="localGridOptions.offsetX" label="X Offset" type="number" density="compact"
                hide-details suffix="px" style="max-width: 120px;" />
              <v-text-field v-model.number="localGridOptions.offsetY" label="Y Offset" type="number" density="compact"
                hide-details suffix="px" style="max-width: 120px;" />
            ...
            <v-switch v-model="localGridOptions.showMajorGridLines" label="Show Major Grid Lines (every 5 squares)"
              color="primary" density="compact" hide-details class="mb-3" />

            <v-switch v-model="localGridOptions.showCoordinates" label="Show Grid Coordinates (A1, B2, etc.)"
              color="primary" density="compact" hide-details class="mb-3" />

            <!-- Diagonal Movement Rule -->
            <div class="mb-3">
              <v-select v-model="localGridOptions.diagonalRule" label="Diagonal Movement Rule" :items="[
              ...
              ]" item-value="value" item-title="title" density="compact" hide-details />
            ...
            <v-switch v-model="localGridOptions.showMeasurementsOnPortal" label="Show Measurements on Portal"
          ...
          <div>
            ...
            <v-switch v-model="localLockBackgroundImages" label="Lock Background Nodes" color="primary"
              density="compact" hide-details class="mb-3" />
            <v-slider v-if="backgroundImageCount > 0" v-model="localBackgroundOpacity" label="Background Opacity"
              min="0" max="1" step="0.05" thumb-label hide-details class="mb-3">
              <template #prepend>
                <v-icon icon="mdi-opacity" />
              </template>
            </v-slider>

            <v-alert v-if="backgroundImageCount > 0" type="info" variant="tonal" density="compact" class="mb-3">
              {{ backgroundImageCount }} background node{{ backgroundImageCount > 1 ? 's' : '' }} on canvas.
            </v-alert>

            <div class="d-flex gap-2 mb-4">
              <v-btn color="primary" variant="outlined" size="small" @click="showFileManager = true"
            ...
            <div class="d-flex gap-2">
              <v-btn color="primary" variant="outlined" size="small" @click="showCanvasBackgroundSelector = true"
                prepend-icon="mdi-image-plus">
                {{ canvasBackgroundUrl ? 'Change' : 'Set Canvas Background' }}
              </v-btn>
              <v-btn v-if="canvasBackgroundUrl" color="error" variant="outlined" size="small"
                @click="removeCanvasBackground" prepend-icon="mdi-delete">
                Remove
              </v-btn>
            </div>
          </div>
          ...
          <div>
            ...
            <v-btn color="primary" variant="tonal" size="small" prepend-icon="mdi-projector-screen" block
              @click="handleSendToPortal" :disabled="!hasActivePortal">
              Send to Portal
            </v-btn>
          </div>
        </v-card-text>
    ...
    <div v-if="showNodeContextMenu" class="context-menu-container"
      :style="{ left: `${contextMenuPosition.x}px`, top: `${contextMenuPosition.y}px` }">
      <v-card min-width="200" class="context-menu-card">
        <v-list density="compact" class="py-1">
          <!-- Duplicate -->
          <v-list-item prepend-icon="mdi-content-copy" title="Duplicate" @click="contextMenuDuplicate" />

          <v-divider class="my-1" />

          <!-- Layer ordering -->
          <v-list-item prepend-icon="mdi-arrange-bring-to-front" title="Bring to Front"
            @click="contextMenuBringToFront" />
          <v-list-item prepend-icon="mdi-arrange-send-to-back" title="Send to Back" @click="contextMenuSendToBack" />
          ...
          <v-menu location="end" open-on-hover :close-on-content-click="true">
            <template #activator="{ props: menuProps }">
              <v-list-item v-bind="menuProps" prepend-icon="mdi-layers-outline" title="Move to Layer"
                append-icon="mdi-chevron-right" />
            </template>
            <v-card min-width="180" class="context-menu-card">
              <v-list density="compact" class="py-1">
                <v-list-item v-for="layer in layers" :key="layer.id" :title="layer.name"
                  :prepend-icon="layer.id === contextMenuItemLayer ? 'mdi-check' : 'mdi-layers'"
                  :disabled="layer.id === contextMenuItemLayer" @click="contextMenuMoveToLayer(layer.id)" />
              </v-list>
            </v-card>
          </v-menu>
          ...
          <!-- Delete -->
          <v-list-item prepend-icon="mdi-delete" title="Delete" class="text-error" @click="contextMenuDeleteItem" />
        </v-list>
      </v-card>
    </div>

    <!-- Pane Context Menu (right-click on canvas) -->
    <div v-if="showPaneContextMenu" class="context-menu-overlay" @click="closeContextMenus"
      @contextmenu.prevent="closeContextMenus" />
    <div v-if="showPaneContextMenu" class="context-menu-container"
      :style="{ left: `${contextMenuPosition.x}px`, top: `${contextMenuPosition.y}px` }">
      <v-card min-width="220" class="context-menu-card">
        <v-list density="compact" class="py-1">
          ...
          <!-- Add Text Note -->
          <v-list-item prepend-icon="mdi-text-box-plus-outline" title="Add Text Note" @click="contextMenuAddTextNode" />

          <!-- Add Shape -->
          <v-list-item prepend-icon="mdi-shape-plus" title="Add Shape" @click="contextMenuAddShapeNode" />

          <!-- Add User File -->
          <v-list-item prepend-icon="mdi-file-image-plus" title="Add File/Image" @click="contextMenuAddUserFile" />

          <!-- Add DM Screen -->
          <v-list-item prepend-icon="mdi-monitor-dashboard" title="Add DM Screen" @click="contextMenuAddDmScreen" />
          ...
          <v-menu location="end" open-on-hover :close-on-content-click="true">
            <template #activator="{ props: menuProps }">
              <v-list-item v-bind="menuProps" prepend-icon="mdi-creation" title="Add Effect"
                append-icon="mdi-chevron-right" />
            </template>
            <v-card min-width="200" max-height="400" class="context-menu-card overflow-y-auto">
              <v-list density="compact" class="py-1">
                ...
                <v-list-item
                  ...
                  :key="preset.id" :prepend-icon="preset.icon" :title="preset.name" :subtitle="preset.description"
    ...
    <!-- DM Screen Selector Dialog -->
    <v-dialog v-model="showDmScreenSelector" max-width="450" persistent>
      <v-card class="dm-screen-selector-dialog">
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showDmScreenSelector = false">Cancel</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <div v-if="!dmScreen.items || dmScreen.items.length === 0" class="empty-state-overlay">
      <div class="empty-state-content">
        ...
        <div class="empty-state-actions">
          <v-btn color="primary" variant="flat" prepend-icon="mdi-plus" @click="showItemSelector = true">
            Add Item
          </v-btn>
          <v-btn color="secondary" variant="outlined" prepend-icon="mdi-image-plus" @click="showFileManager = true">
...
// Computed list of DM screens available to embed (excludes current screen)
const availableDmScreens = computed(() => {
  return dmScreensStore.dmScreens.filter(
    (ds: DmScreen) => ds.id !== props.dmScreen.id && ds.libraryId === props.dmScreen.libraryId
  )
})

function handleDmScreenSelect(screen: DmScreen) {
  const position = lastContextMenuFlowPosition.value || { x: 100, y: 100 }
  dmScreensStore.addDmScreenNode(
    props.dmScreen.id,
    props.dmScreen.libraryId,
    screen.id,
    screen.name,
...
async function handleAddLibraryItem(libraryItem: LibraryItem) {
  try {
    ...
    if (lastContextMenuFlowPosition.value) {
      // Only override if not already assigned to a special layer (like NOTES or EFFECTS)
      if (newItem.layer !== DEFAULT_LAYERS.NOTES && newItem.layer !== DEFAULT_LAYERS.EFFECTS) {
  ...
  try {
    ...
    if (parsed && parsed.type === 'effect-node') {
      ...
      dmScreensStore.addEffectNode(
        ...
        parsed.targetLayer || DEFAULT_LAYERS.EFFECTS
      )
    ...
    if (textData && textData.startsWith('effect:')) {
      ...
      const targetLayer = parts[2] || DEFAULT_LAYERS.EFFECTS
